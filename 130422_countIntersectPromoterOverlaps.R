...
startUnique = subset(sumCount1, !duplicated(sumCount1$start))
endUnique = subset(startUnique, !duplicated(sumCount1$end))
endUnique = subset(endUnique, endUnique$start != NA)
...
write.table(editCount, './130422_countIntersectPromoterInstance.txt', sep='\t', row.names=F)
write.table(sumCount1, './130422_interSectCountPromoter.txt', sep='\t', row.names=F)
write.table(endUnique, './130422_interSectCountPromoterUnique.txt', sep='\t', row.names=F)