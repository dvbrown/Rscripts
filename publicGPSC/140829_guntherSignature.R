...
##################### IO ###########################
setwd('~/Documents/public-datasets/GPSC_subgroups/Gunther2008-Oncogene/analysis/')
...
heatmap.2(t(bigResult), cexRow=1.2, main="Enrichment of FACS marker signatures \n in Molecular Subtype", scale="none",
          ...
          ColSideColors=as.character(dm$colour), xlab="Gunther et al 2008 samples", labCol=row.names(bigResult), labRow=colnames(bigResult), 
...
heatmap.2(t(smallResult), cexRow=1.5, cexCol=1.5, scale="none", #main="Enrichment of FACS marker signatures \n in Molecular Subtype", 
          Rowv=NULL, Colv=TRUE, keysize=1, trace="none", col=myPalette, density.info="none", dendrogram="column", 
          ColSideColors=as.character(dm$colour), xlab="Gunther et al 2008 samples", labCol=row.names(smallResult), labRow=colnames(smallResult), 
          offsetRow=c(1,1), margins=c(8,7))
...
heatmap.2(markers, cexRow=1.2, main="Enrichment of FACS marker mRNAs \n in Molecular Subtype", scale="none",
          Rowv=NULL, Colv=TRUE, keysize=1, trace="none", col=myPalette, density.info="none", dendrogram="column", 
          ColSideColors=as.character(dm$colour), xlab="Gunther et al 2008 samples", labCol=colnames(markers), labRow=c('CD44', 'CD15', 'CD133'), 
          offsetRow=c(1,1), margins=c(8,7))
