...
countMotifInPromoter <- function (genePromoter, posCountMatrix) {
  ...
  orfs = as.character(mget(genePromoter, org.Hs.egALIAS2EG))
...
motifCount = countMotifInPromoter(genes, pcmTranscriptionFactor)

#Use apply to use the countMotifInPromoter function across a vector
promtoterMotifCounts = apply(genes, 1, countMotifInPromoter, posCountMatrix=pcmTranscriptionFactor)
