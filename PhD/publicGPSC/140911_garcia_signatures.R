...
##################### IO ###########################
setwd('~/Documents/public-datasets/GPSC_subgroups/garcia2010/analysis/')

data = read.delim('140911_garciaGEM.txt', row.names=1)
...
heatmap.2(t(bigResult), cexRow=1.2, main="Enrichment of FACS marker signatures \n in FACS sorted GPSCs", scale="none",
          ...
          ColSideColors=as.character(dm$colour), xlab="Bhat et al 2013 samples", labCol=name, labRow=colnames(bigResult), 
...
heatmap.2(t(smallResult), cexRow=1.2, main="Enrichment of FACS marker signatures \n in FACS sorted GPSCs", scale="none",
          Rowv=T, Colv=as.character(dm$colour), keysize=1, trace="none", col=myPalette, density.info="none", dendrogram="row", 
          ColSideColors=as.character(dm$colour), xlab="Bhat et al 2013 samples", labCol=name, labRow=colnames(smallResult), 
          offsetRow=c(1,1), margins=c(8,7))
...
heatmap.2(markers, cexRow=1.2, main="Enrichment of FACS marker mRNAs \n in FACS sorted GPSCs", scale="row",
          Rowv=T, Colv=as.character(dm$colour), keysize=1, trace="none", col=myPalette, density.info="none", dendrogram="row", 
          ColSideColors=as.character(dm$colour), xlab="Bhat et al 2013 samples", labCol=name, labRow=c('CD44', 'CD15', 'CD133'), 
          offsetRow=c(1,1), margins=c(8,7))