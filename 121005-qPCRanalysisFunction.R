...
mapRawData = function(map, data) {
  ...
  wellPos = read.delim('~/Documents/RNAdata/qPCRexpt/120817-384welllayouttranspose.txt')
...
fixSplitSample = function(dataFrame, sample, mean, sd) {
  sample = dataFrame$sample
  mean = dataFrame$mean.Cp
  sd = dataFrame$sd.Cp
  x = cbind(sample, mean, sd)
  return(x)
}
