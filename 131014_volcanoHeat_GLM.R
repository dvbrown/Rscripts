...
finalResult$threshold = as.factor(abs(finalResult$logFC) > 2 & finalResult$FDR < 0.05)
...
dd_text = finalResult[(abs(finalResult$logFC) > 5) & (finalResult$FDR < 0.0005),]
#add text to volcano
...
data = read.delim('GLMedgeR/131021_shortVSlongDEgenes.txt')
row.names(data) = data$external_gene_id
rawData = read.delim('GLMedgeR/131021_normalisedCPM')
