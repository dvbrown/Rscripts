...
\blindtext vector of these gene names was then used to build the network of coexpressed genes using the manual module construction method.
  ...
  \subsection*{Functional assays}  
    \paragraph {Proliferation curve}\\
    \paragraph {Limiting dilution (ELDA)}\\
    \paragraph {Temozolomide sensitivity}\\
    \paragraph {Invasion}\\


  \subsection*{Library preparation and RNA sequencing}  
     \paragraph {RNA-seq libraries}\\
\blindtext
    
  \subsection*{Bioinformatic analysis} 
    \paragraph {Preprocessing}\\
    \paragraph {Differential expression testing}\\
    \paragraph {Subtype classification}\\
    \paragraph {Gene signature and survival analysis}\\
\blindtext
