...
dataDesign = data.frame(row.names=colnames(sumData)
                        ,condition=c('negative','positive',
                                     'unsorted','test'),
                        libType=c('paired-end','paired-end','paired-end','paired-end'))

plot(sumData[,1], sumData[,2], xlim=c(0,60000), ylim=c(0,60000), cex=0.25, type='p', ylab='CD133 low', xlab='CD133 high', main='Read Count correlation different populations')
lines(lowess(sumData[,1],sumData[,2]), col="blue") # lowess line (x,y) 

conds = dataDesign$condition
#create a count data set object
countDataSet = newCountDataSet(sumData, conds)
