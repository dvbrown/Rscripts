...
\JournalInfo{Nature Communications (10.015) Plos Comp Bio (5.515) or similar biologyish journal} % Journal information
\Archive{Original/ methodological article using public data} % Additional notes (e.g. copyright, DOI, review/research article)

\PaperTitle{In silico investigation of cancer stem markers for flow cytometry in Glioblastoma Multiforme} % Article title

\Authors{Brown, Daniel V\textsuperscript{1}*,  Daniel, Paul M\textsuperscript{1}, People, More\textsuperscript{2}
...
\affiliation{\textsuperscript{2}\textit{Anyone else who helps me along the way, Melbourne, Australia}} % Author affiliation
...
will primarily focus on glioblastoma multiforme but I may throw in Breast and colorectal cancer if it is potentially interesting. I will compare the derivation of a cancer stem cell signature de novo from the data itself based on coexpression. 
I will compare this to signatures from the the literature and also known pathways. I will use this signature topredict survival and the examine if a marker defines a particular subtype. 
Maybe pad it out at the end with some qRT-PCR, but I will need primary tissue from somewhere!}
