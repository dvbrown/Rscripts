...
This filter was \emph{weighted} 0.125 < 1. Weighted is the measure of network adjacency. Invert selection was used and then edges were hidden. After this nodes that were connected by the edges left over were selected. The invert selection to hidden features were used to trim the netwrk.
...
The PROM1 gene was processed with WGCNA using the Agilent microarray data from the TCGA glioblastoma (see supplemental for why Agilent was selected). This sections describes the pattern of coexpressed genes and outlines the overall approach.
...
To this end we identified the most significantly correlated genes with CD133 (PROM1) gene with a cutoff of \textbf {greater than 2 standard deviations from the mean and less than 0.05 FDR corrected p-value} (figure 1). It was established that the correlation datapoints were normally distributed (figure 2).\\

The most critical parameter was the cutoff used to build the model of coexpressed genes. A cutoff of 2 plus or minus standard deviations and a FDR less than 0.05 was used to obtain the list of genes that are significantly coexpressed with the markers of interest. This is about 600 genes for any marker. 
The consequence of this is the top genes are roughly the same number but the magnitude of them are different. In the case of CD44 the correlation values are much higher than for CD133.
...
The set of \textbf {approx 600} genes that were significantly coexpressed were subsequently analysed by weighted coexpression analysis to identify the patterns of coexpression. Supplying only a subset of expressed genes also greatly decreased the run time of the network adjacency algorithm.
...
Next step is to map correlation to the network to figure out positively and negatively correlated genes.

\begin{figure}[ht]
   \centering 
% Change this plot when you change the data source   
    \includegraphics[page=1, width=.95\linewidth]{/Users/d.brown6/Documents/public-datasets/stddata_2013_12_10/GBM/20131210_dataReformatting/dataRearranging/wgcna/manualCorrelation/networks/CD133_CytoNodeV2.png}
    \includegraphics[page=1, width=.95\linewidth]{/Users/d.brown6/Documents/public-datasets/stddata_2013_12_10/GBM/20131210_dataReformatting/dataRearranging/wgcna/manualCorrelation/networks/CD44_CytoNode.png}
    \caption{{The network obtained from coexpression of CD133 (top) and CD44 (bottom)}  }
    \label{fig:network}
\end{figure}
