...
print(input)
 sampletostudy = as.character(samplelist[input])
    print(sampletostudy)
...
jpeg(paste("../genomewideProfilesLogR500K/LogR_", gsub(".sorted_dedup._mappable_500K_89bases.sorted.count","",sampletostudy),".jpeg",sep=""),width = 1200, height = 600, units = "px", pointsize = 20)	
...
title<-paste("LogR", gsub(".sorted_dedup._mappable_500K_89bases.sorted.count","",sampletostudy))
plot(c(1:length(data)),data, pch=19,cex=0.3,col="black",type="p", ylim=c(min-0.5,max+0.5), xlab = "Chromosomes", ylab = "logR ", main=title ,xaxt = "n",cex.lab=1.2)
