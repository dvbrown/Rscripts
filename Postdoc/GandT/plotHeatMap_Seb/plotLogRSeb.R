### Script to make CNV heatmaps in downsampled bins

#### necessary arguments to fill in

basedir             <- "/Users/u0107775/Data/GandT_Seq/170925_MEL06_GandT/171107_MEL06_DNA/Results/results_ploidy2_ref2/bins/PassQC/"
locationSampleDescr <- "/Users/u0107775/Data/GandT_Seq/170925_MEL06_GandT/171107_MEL06_DNA/171212_qcScores.txt"
interest            <- "Sample_ID"
title               <- "MEL06_PDX.png"

#### filtering on MAPD score
...
for (i in 1:length(myfiles) ) {
    ...
    # DANIEL Changed from column 5 to column 7 to switch to plotting logR
    DT[[myfiles[i]]] <- test[,c(8,7)]
    # DANIEL changed this line below to get only the GC code and index
    cnts <- as.character(substr(myfiles[i],23,48))
    colnames(DT[[myfiles[i]]]) <- c("ID", cnts)
}

###display number of bins per chr => determine number of bins you need to take together to get a intepretable heatmap (preferable 25 bins for chr 1)
bins=test[,"chr"]
tb_bin=table(bins)
tb_bin[1]
...
palette <- c("gray95", "gray13", "gold2", "plum4", "darkorange1", "lightskyblue2", "firebrick", "burlywood3", "gray51", "springgreen4", "lightpink2", "deepskyblue4", "lightsalmon2", "mediumpurple4", "orange", "maroon", "yellow3", "brown4", "yellow4", "sienna4", "chocolate", "gray19", "thistle1")
...
##### Create scater object that contains the CNV matrix and a sampledescription file ####
cellIdentifiers <- read.table(locationSampleDescr, header=TRUE, row.names=1, sep="\t")
cellIdentifiers$TT=rownames(cellIdentifiers)

# MAPD filtering
filter=rownames(cellIdentifiers[(cellIdentifiers[,columnMAPD]<max & cellIdentifiers[,columnMAPD]>=min & !is.na(cellIdentifiers[,columnMAPD]) ),])
Data2=Data2[,colnames(Data2) %in% filter]

samplesToAnalyze=colnames(Data2)
cellid=data.frame(cellIdentifiers[rownames(cellIdentifiers) %in% samplesToAnalyze,])
cellid$TT=NULL

N=length(unique(cellid[,interest]))+1
palette = brewer.pal(N, "Set1")
xx=levels(cellid[,interest])[1:N-1]
names(palette)=levels(cellid[,interest])
l2cols <- palette[as.integer(factor(cellid[,interest], levels = xx))]
...
col_breaks = c(seq(-1,-0.49,length=100),  # for red
               seq(-0.5,0,length=100),           # for yellow
               seq(0.1,2,length=100))             # for blue

png(title,    # create PNG for the heat map        
...
heatmap.2(test,
          #main = "CNV heatmap", # heat map title
