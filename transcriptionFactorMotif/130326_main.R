...
option_list <- list(
    ...
    make_option(c("-g", "--geneList"), action="store",type = 'character', default='~/Documents/CREB/transcriptionFactorAnalysis/130325_TFCREB/130326_',
                help="A tab delimited text file of listing the geneSymbols of the promoters you wish to interrogate"),
    make_option(c("-d", "--databaseNo"), action="store", type='integer', default=2,
                help="The database number of the transcription factor motif you wish to use. Check using the motifPlotting script"),
...
pcmTranscriptionFactor = getPositionCountMatrix(transcriptionFactor, dataBaseToUse)

#Use apply to use the countMotifInPromoter function across the vector of geneIDs
promtoterMotifCounts = apply(orfs, 1, countMotifInPromoter, posCountMatrix=pcmTranscriptionFactor, up=upStream,down=downStream)
