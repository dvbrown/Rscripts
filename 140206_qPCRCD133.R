...
test1 = ggplot(cData, aes(x=gene.x, y=mean, fill=cd133)) + 
...
da = cData[!cData$gene.x %in% c('HAPLN1'),]
test2 = ggplot(da, aes(x=gene.x, y=mean, fill=cd133)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9)) +    
...
pdf('./qcplots/ggPlot/140211_cd133T_testErrorBar.pdf')
multiplot(test1, test2)
dev.off()